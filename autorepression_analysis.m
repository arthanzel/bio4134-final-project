...
f1 = figure(1);
hold on;
fn = @(t, x) autorepression(x, 0.1, 0.9, 1, 0.2, 1);
vals = [0 2 4 6];
for x0_ = vals
...
legend(num2str(vals'));
saveas(f1, 'figures/autorepression-base.png');
...
vals = 0:0.5:2;
for ai = vals
    fn = @(t, x) autorepression(x, 0.1, ai, 1, 0.2, 1);
    [t, y] = ode45(fn, [0, tFinal], x0);
    plot(t, y);
end
cttitle('Protein concentration over time for varying values of \alpha');
legend(num2str(vals'));
saveas(f3, 'figures/autorepression-ai.png');
...
vals = 10 .^ (-2:2);
for kd = vals
    fn = @(t, x) autorepression(x, 0.1, 0.9, kd, 0.2, 1);
    [t, y] = ode45(fn, [0, tFinal], x0);
    plot(t, y);
end
cttitle('Protein concentration over time for varying values of K_D');
legend(num2str(vals'));
saveas(f4, 'figures/autorepression-kd.png');
...
vals = 0.1:0.2:1;
for d = vals
    fn = @(t, x) autorepression(x, 0.1, 0.9, 1, d, 1);
    [t, y] = ode45(fn, [0, tFinal], x0);
    plot(t, y);
end
cttitle('Protein concentration over time for varying values of \delta');
legend(num2str(vals'));
saveas(f5, 'figures/autorepression-d.png');
...
vals = 1:4;
for n = vals
    fn = @(t, x) autorepression(x, 0.1, 0.9, 1, 0.2, n);
    [t, y] = ode45(fn, [0, tFinal], x0);
    plot(t, y);
end
cttitle('Protein concentration over time for varying values of n');
legend(num2str(vals'));
saveas(f6, 'figures/autorepression-n.png');
...
legend(num2str(vals'));
saveas(f7, 'figures/autorepression-nneg.png');
